...
import torch as t
from data.voc_dataset import VOCBboxDataset, VOCBboxDataset_test, VOCBboxDataset_test_all
...
class Dataset:
    ...
    def __getitem__(self, idx):
        ...
        return img.copy(),  ori_img.shape[1:], bbox.copy(), label.copy(), scale, id_
...
class TestDataset_all:
    # 测试集 有五个输出
    def __init__(self, opt, split=opt.testtxt, use_difficult=True):
        self.opt = opt
        self.db = VOCBboxDataset_test_all(
            opt.voc_data_dir, split=split, use_difficult=use_difficult)
