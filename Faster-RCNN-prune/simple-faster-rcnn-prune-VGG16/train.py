...
from model import FasterRCNNVGG16_SPARSITY
...
def train(**kwargs):
    ...
    if opt.sr:
        ...
        history_score = np.zeros((opt.epoch - 0 + 1, 3))
        faster_rcnn = FasterRCNNVGG16_SPARSITY()
    else:
        print("进入未稀疏化的代码")
    ...
    for epoch in range(opt.epoch):
        ...
        for ii, (img, bbox_, label_, scale) in tqdm(enumerate(dataloader)):
            ...
            trainer.train_step(trainer, img, bbox, label, scale)
        ...
        log_info = 'lr:{}, map:{},loss:{}'.format(str(lr_), str(eval_result['ap']), str(eval_result['map']), str(trainer.get_meter_data()))
